...
func deriveHeaderFor(masterKey []byte, fileID string, chunkSize int) (stagedHeader, error) {
	...
	salt, err := hkdfBytes(16, masterKey, fileIDbytes, []byte("upload-salt:v1"))
	if err != nil {
		return stagedHeader{}, fmt.Errorf("derive salt: %w", err)
	}
	nonce, err := hkdfBytes(8, masterKey, fileIDbytes, []byte("upload-nonceprefix:v1"))
	if err != nil {
		return stagedHeader{}, fmt.Errorf("derive nonce: %w", err)
	}
	hdr := encryption.GenerateHeader(chunkSize, salt, nonce)
	return stagedHeader{hdr: hdr, salt: salt, noncePrefix: nonce}, nil
}
func hkdfBytes(n int, key, salt, info []byte) ([]byte, error) {
...
func encryptRecord(masterKey []byte, sh stagedHeader, index uint32, plain []byte) ([]byte, error) {
	key, err := encryption.DeriveFileKey(masterKey, sh.salt)
	if err != nil {
		return nil, err
	}
	aead, err := encryption.GetGCMBlock(key)
	if err != nil {
		return nil, err
	}
...
func assemble(masterKey []byte, baseDir, userID, logicalPath, staging string, sh stagedHeader, totalChunks int, totalSize int64) (string, error) {
	defer os.RemoveAll(staging) // always clean up staging

	// allocate final path & manifest entry *now*
	dstPath, err := ResolveForCreate(masterKey, baseDir, userID, logicalPath)
	if err != nil {
		return "", err
	}

	// create final file; write header
	out, err := os.OpenFile(dstPath, os.O_CREATE|os.O_WRONLY|os.O_EXCL, 0644)
	if err != nil {
		return "", err
	}
	...
	if totalSize > 0 {
		if err := UpdateFileMeta(masterKey, baseDir, userID, logicalPath, totalSize, time.Now()); err != nil {
			return "", fmt.Errorf("assembled but metadata update failed: %w", err)
		}
	}
...
func IngestChunkStateless(masterKey []byte, baseDir, userID string, meta ChunkMeta, plain []byte) (assembled bool, assembledLogicalPath string, err error) {
	if meta.ChunkSize <= 0 {
	...
	lp, err := assemble(masterKey, baseDir, userID, meta.LogicalPath, staging, sh, meta.TotalChunks, meta.TotalSize)
	if err != nil {
		return false, "", err
	}
	return true, lp, nil
